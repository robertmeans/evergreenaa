...
function validate_new($row, $nf1, $fn1, $nf2, $fn2, $nf3, $fn3, $nf4, $fn4) {
  ...
  /* difference between new and update right here... if new file is not set but there's a name/label present, let them know. In the update version they are allowed to change the name because it sees that a hidden field with the file name is present. */
...
/* different validation function below because on edit you have to allow for a new file label to be entered without adding a new file. If they delete the name that's the same as deleting the file too because you cannot add a name without also adding a file. In other words, the validate_new() (above) mandates that you have a file attached if you have a file name. This one (below) fills in the file name via $row['link1'] and lets you change it. If you delete the name on edit you cannot then come back and add just a name and have it reconnect with the file - it will instead tell you to select a file. it's a cluster but trust yourself - this is right... unless it's not. */
/*
...more info even though you didn't ask for it:
the validate_new() function also only needs 1 instance of each file position (with 3 considerations of each position: 1. was there a file)
*/
