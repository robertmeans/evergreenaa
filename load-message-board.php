...
?>
    ...
    <form id="<?= $i ?>" class="mbform" action="post.php?post-id=<?= $row['idt_topic'] ?>" method="get">
      <input id="pid_<?= $i ?>" type="hidden" name="post-id" value="<?= $row['idt_topic'] ?>">
      ...
      <?php } else if (isset($_SESSION['id']) && ($_SESSION['id'] != $row['idt_user'])) { // logged in but not theirs so put 'Read & Reply' in tooltip with class of right ?>
    ...
    <?php if (isset($_SESSION['mode']) && ($_SESSION['mode'] == 1 && ($_SESSION['admin'] == 1 || $_SESSION['admin'] == 3))) { ?>
      ...
      <?php } else { ?>
        <a class="gtp" href="user_role.php?user=<?= h(u($row['idt_user'])); ?>"><div class="tooltip"><span class="tooltiptext">Manage User</span><i class="far fas fa-user-cog"></i></div></a>
      <?php } ?>
    ...
    <?php /* end delete icon */ ?>
		...
		<?php if (isset($_SESSION['id']) && ($_SESSION['mode'] == 1 && ($_SESSION['admin'] == 1 || $_SESSION['admin'] == 3))) { ?>
			...
			<?php } else { ?>
				<a class="admin-mb-info gtp" href="user_role.php?user=<?= h(u($row['idt_user'])); ?>"><div class="tooltip"><span class="tooltiptext">Manage User</span><?= $row['username'] . ' &bullet; ' . $row['email'] ?></div></a>
